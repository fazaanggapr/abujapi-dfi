...
import { useParams } from "react-router-dom"; // Mengambil parameter ID dari URL
...
function EditLocation() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("access_token");
      const res = await fetch(`${baseUrl}/location/edit/${id}`, {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Gagal memperbarui lokasi.");
      }
  ...
  return (
    <div className="max-w-xl mx-auto mt-10 bg-white rounded-xl shadow-lg border border-gray-200">
      <div className="px-8 py-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Edit Lokasi</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="text-right">
            ...
            >
              {isSubmitting ? "Menyimpan..." : "Perbarui Lokasi"}
            </button>
          </div>
        </form>
      </div>
